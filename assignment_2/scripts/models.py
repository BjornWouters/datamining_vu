...
def import_data():
    training_set = pd.read_csv("../results/kaggle_train.csv", sep=',')
    original_dataset = pd.read_csv("../results/kaggle_dataset.csv", sep=',')
    ...
    test_set = pd.read_csv("../results/kaggle_test.csv", sep=',').fillna(0)
    test_set = test_set.drop(['prop_starrating'], axis=1)
    return X_train, y_train, X_validation, original_dataset, test_set


def GBM(X, y):
    clf = GradientBoostingRegressor(random_state=None, n_estimators=1500, max_depth=2,
...
def RF(X, y):
    clf = RandomForestRegressor(n_estimators=10, max_depth=None, min_samples_split=2,
                                random_state=4, max_features='sqrt', n_jobs=1)
    clf.fit(X, y)
